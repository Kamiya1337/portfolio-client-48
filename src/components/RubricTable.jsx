        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-3">
          <div>
            <h3 className="font-bold text-slate-800 mb-1 flex items-center gap-2">
              <RefreshCw size={16} className="text-academic-blue animate-spin" style={{ animationDuration: '8s' }} />
              Portfolio Readiness Score
            </h3>
            <p className="text-sm text-slate-500">
              Trạng thái minh chứng thật: Đã tích hợp thành công <span className="font-bold text-academic-blue bg-academic-sidebar px-2 py-0.5 rounded border border-academic-border">{completedMilestones} trên {totalMilestones}</span> mục dữ liệu.
            </p>
          </div>
          <span className="text-3xl font-black text-academic-blue tracking-tight">{progressPercent}%</span>
        </div>
        <div className="w-full bg-academic-sidebar rounded-full h-3.5 overflow-hidden border border-academic-border">
          <div 
            className="bg-gradient-to-r from-academic-blue to-academic-blue-light h-full rounded-full transition-all duration-700 ease-out" 
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[800px]">
            ...
            <tbody className="divide-y divide-slate-200 text-sm">
              ...
              <tr className="hover:bg-academic-sidebar transition-colors">
                <td className="p-4 font-bold text-slate-800 flex items-start gap-2">
                  <Target className="text-academic-blue mt-0.5 flex-shrink-0" size={16} />
                ...
                <td className="p-4">
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded text-xs font-bold border bg-academic-sidebar text-academic-blue border-academic-border">
                    <CheckCircle2 size={14} className="text-academic-blue" /> Hoàn thành Layout
                  </span>
                </td>
              ...
              {projects.map((project) => {
                ...
                return (
                  <tr key={project.id} className="hover:bg-academic-sidebar transition-colors">
                    <td className="p-4 font-bold text-slate-800 flex items-start gap-2">
                      <Target className="text-academic-blue mt-0.5 flex-shrink-0" size={16} />
                    ...
                    <td className="p-4">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded text-xs font-bold border ${
                        isFullyDone 
                          ? "bg-academic-sidebar text-academic-blue border-academic-border" 
                          : "bg-amber-50 text-amber-700 border-amber-100"
                      }`}>
                        <CheckCircle2 size={14} className={isFullyDone ? "text-academic-blue" : "text-amber-600"} /> 
